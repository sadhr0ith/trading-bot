...
- [x] Add `src/trading_bot/strategies/atr_breakout_strategy.py`
...
- [x] Add config `src/trading_bot/configs/config_atr_breakout.py`
- [x] Wire into `src/trading_bot/strategy_manager.py` registry.
- [x] Add backtest adapter implementation.

Acceptance:
- Backtest runs and produces trades; no lookahead; stable results.

### 2.2 Mean Reversion Strategy (15m/1h)
- [x] Add `src/trading_bot/strategies/mean_reversion_strategy.py`
...
- [x] Add config `src/trading_bot/configs/config_mean_reversion.py`
- [x] Wire into registry + backtest.

### 2.3 Regime Switcher (ADX/vol filter)
- [x] Add `src/trading_bot/strategies/regime_switch_strategy.py`
  - Compute regime: trend vs range vs high-vol no-trade.
  - Route to ATR breakout or mean reversion.
- [x] Config includes thresholds and cooldowns.
...
- [x] For intraday ML, define label as forward return:
...
- [x] Add `src/trading_bot/strategies/day_trading_ml_strategy.py`
...
- [x] Add “no-trade when uncertain” behavior.
...
- [x] Add `python -m trading_bot.train --strategy day_trading_ml` command.
- [x] Ensure live loop runs `inference_only=True` by default for ML strategies.

### 4.2 Experiment tracking (lightweight)
- [x] Persist per-run metadata: dataset hash, feature version, horizon, costs, metrics.
- [x] Write reports to `reports/` with timestamp + config signature.

### 4.3 Observability & drift
- [x] Add a small “data health” report per run:
...
- [x] Add universe loader (top N by volume, cached).
- [x] Add portfolio constraints:
  - max positions
  - per-asset max exposure
  - correlation/vol targeting (simple first)
- [x] Add portfolio-level backtest metrics.
